...
fn deploy_shd(user_a: String, router: Contract) -> Result<(SiennaMarketOracleContract)> {
    ...
    let sienna_scrt_shd = Contract::new(
        SIENNA_SHD_SSCRT_PAIR.to_string(),
        SIENNA_SHD_SSCRT_PAIR_HASH.to_string(),
    );
