...
use secretcli::utils::generate_label;
use serde::Serialize;
...
fn main() -> Result<()> {
    ...
    //
    // let stkd_scrt_shd_lp_oracle = deploy_stkd_scrt_shd_lp(user_a.clone(), router.as_contract())?;
    // router.update_oracle(HOOMP_KEY, "SILK", silk_oracle.as_contract())?;
    // router.update_oracle(HOOMP_KEY, "SHD", shd_oracle.as_contract())?;
    // router.update_oracle(HOOMP_KEY, "stkd-SCRT", stkd_scrt_oracle.as_contract())?;
    // router.update_oracle(
    //     HOOMP_KEY,
    //     "stkd-SCRT/SCRT SiennaSwap LP",
    //     stkd_scrt_scrt_lp_oracle.as_contract(),
    // )?;
    ...
    //     RegistryOperation::UpdateAlias {
    //         alias: keys::STKD_SCRT_SHD_LP.to_string(),
...
fn deploy_stkd_scrt(
    ...
    let stkd_scrt_oracle = ShadeStakingDerivativeOracleContract::new(
        ...
        Some(HOOMP_KEY),
        Some("stkd_scrt_oracle_2"),
        "stkd_scrt_oracle_shade_0.2",
    )?;
...
fn deploy_sscrt_shd_lp(user_a: String, router: Contract) -> Result<SiennaswapSpotLpOracleContract> {
    ...
    let stkd_scrt_scrt_lp_oracle = SiennaswapSpotLpOracleContract::new(
        ...
        Some(HOOMP_KEY),
        Some("shd_scrt_lp_oracle_2"),
        "sienna_shd_sscrt_lp_oracle_shade_0.2",
    )?;
